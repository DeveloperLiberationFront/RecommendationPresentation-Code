 ...
 * dispatching those events to event listeners registered via the {@link #addUsageDataEventListener(UsageDataEventListener)} method.
 * <p>
 * The instance starts monitoring activities immediately after it is started, but does not dispatch the resulting events until after the workbench is running (as reported by the
 * {@link EclipseStarter#isRunning()} method.
 * </p>
 * <p>
 * Efforts have been taken to try and keep the impact on the user experience&mdash;performance in particular&mdash;to a minimum. Any decision balancing absolute correctness of data
 * capture and user experience is made in favour of preserving positive user experience and reducing any negative impact on performance. In that regard, for example, cancel really
 * means cancel to the {@link #eventConsumerJob} and may leave some events undispatched to the listeners.
 * 
 * @author Wayne Beaton
 */
...
public class UsageDataService {
     ...
     * what the user is up to. The objects in this list are of type {@link UsageMonitor}. Strictly speaking this is not a list of
     ...
     * in anticipation of performance issues (see {@link #startEventConsumerJob()} for discussion.
     */
    ...
    /**
     * Start the {@link #eventConsumerJob}. Various monitors add events to the {@link #events} queue. In order to avoid degrading system performance any
     * more than necessary, events are added to the queue by the monitors. The {@link #eventConsumerJob} then consumes the events from the queue and
    ...
    /**
     * <p>
     * This method queues an event containing the given information for processing.
     * </p>
     * 
     * @param what
     *            what happened? was it an activation, started, clicked, ... ?
    ...
    private void dispatchEvent(UsageDataEvent event) {
        ...
        if (event.bundleVersion == null)
            event.bundleVersion = getBundleVersion(event.bundleId);
    ...
    private void dispatchEvent(UsageDataEvent event, UsageDataEventListener listener) {
        try {
            ...
            UsageDataCaptureActivator.getDefault().logException(
                    "The listener (" + listener.getClass() + ") threw an exception", e); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
